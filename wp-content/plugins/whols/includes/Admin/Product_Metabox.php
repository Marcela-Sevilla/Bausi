...
class Product_Metabox {
    ...
    public function add_metabox_to_previous_metabox_panel(){
        ...
        woocommerce_wp_checkbox(array(
            ...
            'label'       => esc_html__( 'Wholesale Only', 'whols' ),
            'description' => esc_html__( 'Mark this product as a wholesale only product', 'whols' ),
        ));
        echo '</div>';
            ...
            <p class="form-field whols_product_meta_type_1_pricing">
                ...
                <span class="wrap whols_product_meta_wrap">
                    ...
                    <span class="whols_field_wrap">
                        <input name="whols_price_type_1_price" placeholder="<?php echo esc_html__('Price', 'whols') ?>" class="" type="number" step="any" min="0" value="<?php echo esc_attr( $price_type_1_price ); ?>" />
                    </span>
    ...
    public function simple_product_meta_fields_save( $post_id ){
        ...
        if( $pricing_model == 'single_role' ){
            ...
            if ( isset($_POST['whols_price_type_1_price']) ){
                $price = floatval( $_POST['whols_price_type_1_price'] );
            } else {
                $price = '';
            }

            if ( !empty( $price ) ){
                $meta_field_value .= $price. ':' .$min_quantity;
            }
            ...
            <p class="form-field whols_product_meta_type_1_pricing">
                ...
                <span class="wrap whols_product_meta_wrap">
                    ...
                    <span class="form-row form-field whols_field_wrap">
                        ...
                        <input name="whols_price_type_1_price_<?php echo esc_attr($variation->ID); ?>" placeholder="<?php echo esc_attr__( 'Price', 'whols' ); ?>" class="" type="number" step="any" min="0" value="<?php echo esc_attr( $price_type_1_price ); ?>" />
                    </span>
    ...
    public function product_data_variation_fields_save( $post_id ){
        ...
        if( $pricing_model == 'single_role' ){
            ...
            if ( isset($_POST['whols_price_type_1_price_'. $post_id]) ){
                $price = floatval( $_POST['whols_price_type_1_price_'. $post_id] );
            } else {
                $price = '';
            }
