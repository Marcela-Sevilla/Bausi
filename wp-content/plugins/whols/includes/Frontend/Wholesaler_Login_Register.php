...
class Wholesaler_Login_Register{
    ...
    public function registration_form_shortcode( $atts ){
        $nonce = wp_create_nonce( 'whols_register_nonce' );
        ob_start();
        ?>
                    ...
                    <form action="whols_registration_action">
                        <?php
                            ...
                            foreach( $fields as $key => $field ){
                                ...
                                whols_form_field( $key, array( 'type' => $type, 'label' => $label, 'placeholder' => $placeholder, 'description' => $description, 'required' => $required, 'is_additional' => $is_additional, 'options' => $options ), $value );
                            }
                        ?>
                        ...
                        $.ajax({
                            ...
                            success: function( response ){
                                if ( response.registerauth == true ){
                                    ...
                                    $('#whols_user_reg_message').html('<div class="woocommerce"><div class="woocommerce-notices-wrapper"><div class="whols_success_msg woocommerce-message">'+ response.message +'</div></div></div>').fadeIn();
                                    if( redirect_url.length ){
                                        document.location.href = redirect_url;
                                    }
                                }else{
                                    $('#whols_user_reg_message').html('<div class="woocommerce"><div class="woocommerce-notices-wrapper"><div class="whols_invalid_msg woocommerce-error">'+ response.message +'</div></div></div>').fadeIn();
                                }
                            },
