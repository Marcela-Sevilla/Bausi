...
if( !whols_get_option('enable_wholesale_store') ){
    return;
}

add_action( 'init', 'whols_hide_prices_guest_users' );
function whols_hide_prices_guest_users(){
    ...
    if( $hide_price_for_guest_users && !is_user_logged_in() ){
        ...
        add_filter( 'woocommerce_is_purchasable', 'whols_disable_purchasable_guest_users' );

        // finally add custom message instead of showing price & cart button
        add_filter( 'woocommerce_get_price_html', 'whols_filter_woocommerce_get_price_html', 10, 2 );
    }
}
...
/*=====  End of Hide price for guest users  ======*/
 ...
 * @since 1.0.0
 */
add_action( 'woocommerce_before_calculate_totals', 'whols_recalculate_cart_price' );
...
/*=====  End of Apply cart quantity  ======*/


/*==========================================================================
=            Product wholesale price for loop/details & override            =
==========================================================================*/

add_filter( 'woocommerce_get_price_html', 'whols_woocommerce_get_price_html', 10, 2 );
function whols_woocommerce_get_price_html( $price, $product ){
    $current_user_id = get_current_user_id();
    if( is_admin() || !whols_is_wholesaler( $current_user_id ) || !$price > 0 ){
        return $price;
    }
    ...
    if( whols_is_wholesaler( $current_user_id ) && $enable_this_pricing && $price_value ){
        ...
        <div class="whols_minimum_quantity_notice">
            ...
            <?php printf(
                    /*
                     * translators
                     * 1: minimum quantity label
                     */
                    __( 'Wholesale price will apply for minimum quantity of %1$s products.', 'whols' ),
                    $minimum_quantity
                ); ?>
            </span>
        </div>
...
function whols_get_wholesaler_price( $price_type, $price_value, $product = '' ){
    ...
    if( $product_type == 'simple' ){
        if( $price_type == 'flat_rate'){
            ...
            $wholesaler_price = wc_price( whols_get_percent_of( $retailer_price, $price_value ) );
        }
...
function whols_get_price_save_info( $price_type, $price_value, $product = '' ){
    ...
    if( $product_type  ==  'simple' ){
        if( $price_type == 'flat_rate' ){
            ...
            $save_info  = wc_price( $save_price ) . ' ('. round(whols_get_discount_percent( $retailer_price, $price_value )) .'%)';
        } else {
            ...
            $save_info            = wc_price( $save_price ) . ' ('. round(whols_get_discount_percent( $retailer_price, $new_wholesaler_price )) .'%)';
        }
    } elseif( $product_type == 'variable' ){
...
/*=====  End of Product wholesale price for loop/details & override  ======*/


/*====================================================
=            Variable product price tier            =
====================================================*/

add_filter( 'woocommerce_available_variation', 'whols_price_tier', 10, 3 );
function whols_price_tier( $data, $product, $variation ){
    $current_user_id = get_current_user_id();
...
/*=====  End of Variable product price tier  ======*/
...
add_filter( 'woocommerce_coupons_enabled', 'whols_hide_coupon_field_on_cart' );
function whols_hide_coupon_field_on_cart( $enabled ) {
    $current_user_id = get_current_user_id();
    if( !whols_is_wholesaler( $current_user_id ) ){
        return $enabled;
    }
    ...
    if( class_exists('WP_Term_Query') ){
        ...
        foreach ( $term_query->get_terms() as $term ) {
            if( $term->slug ==  $current_user_role ){
...
/*=====  End of Disable coupon for wholesale customers  ======*/
...
add_filter( 'woocommerce_product_query_meta_query', 'whols_exclude_products', 10, 2 );
function whols_exclude_products( $meta_query, $query ) {
    $current_user_id = get_current_user_id();
...
/*=====  End of Exclude products from archive page  ======*/
...
add_filter( 'woocommerce_shipping_free_shipping_is_available', 'whols_shipping_control', 10, 3 );
function whols_shipping_control( $is_available, $package, $shipping_method ){
    $has_free_shipping = 0;
    $allow_free_shipping = '';
...
/*=====  End of Free shipping control for wholesaler  ======*/

/*===========================
=            Tax            =
===========================*/

add_action( 'woocommerce_before_calculate_totals', 'whols_calculate_tax', 100, 1 );
function whols_calculate_tax( $cart ) {
    global $woocommerce;
...
add_filter( 'woocommerce_get_item_data', 'whols_filter_get_item_data', 99, 2 );
function whols_filter_get_item_data( $item_data, $cart_item ){
    ...
    if( whols_is_wholesaler(get_current_user_id()) && $enable_this_pricing && $price_value &&  $cart_item['quantity'] >= $minimum_quantity  ){
        $item_data[] = array(
            'name'      => 'Wholesale',
